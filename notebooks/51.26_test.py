...
class Model(pl.LightningModule):
    ...
    def __init__(
        ...
        num_elements_query: int = 128,
        num_heads: int = 16,
        num_latent_transformer_encoder_layers: int = 4,
        num_elements_into_output_layers: Optional[int] = 32,  #: The subset of elements to take from the latent query into the output layers.  If None then the same as num_elements_query
        dropout: float = 0.0,
...
#model = Model()
model = Model.load_from_checkpoint("/home/jack/dev/ocf/predict_pv_yield/notebooks/.neptune/Untitled/PRED-776/checkpoints/epoch=29-step=251999.ckpt")
